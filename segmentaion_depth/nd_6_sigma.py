...
def nd_6_sigma(img: np.ndarray, **kwargs) -> np.ndarray:
    ...
    _min = round(_height * _width * _s_yield, 3) # min value

    _value = np.unique(img)
    _hist = np.zeros(int(np.max(_value + 1)), dtype=np.uint32)
    ...
    for i in range(_height):
        for j in range(_width):
            if _hist[img[i][j]] < _min and img[i][j] >= _id:
                img[i][j] = _value[int(len(_value)/2)]
